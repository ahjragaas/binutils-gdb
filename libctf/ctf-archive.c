...
ctf_arc_find_magic (unsigned char *buf, size_t len, int *strtab)
{
  ...
  if ((ret = memmem (buf, MIN (len, 65 + sizeof (magic_16)) - sizeof (magic_16),
		     &magic_16, sizeof (magic_16))) != NULL)
    return ret;
  ...
  magic = CTFA_MAGIC;
  if ((ret = memmem (buf, MIN (len, 65 + sizeof (magic)) - sizeof (magic),
			 ...
			 ctf_error_t *errp)
{
  ...
  /* Set the CU name if unset, either from the passed-in name, or by using a
     linker-set default, or by looking up the name via the index.  v4 dicts
     have no intrinsic cuname: this path sets the name for all of them from
     the archive member name (if any) or from the linker-set name (which is
     ultimately how all these names get set in the first place).  */
  if (ctf_dict_cuname (fp) == NULL)
    {
      if (name != NULL
	  && strcmp (name, _CTF_SECTION) != 0)
	ctf_dict_set_cuname (fp, name);
    }
  if (offset == 0 && arci->ctfi_default_parent_cuname)
    ctf_dict_set_cuname (fp, arci->ctfi_default_parent_cuname);
  else if (name == NULL
	   && ((name = ctf_dynhash_lookup (arci->ctfi_member_names,
					   (void *) offset)) != NULL))
    ctf_dict_set_cuname (fp, name);
  return fp;
}
	       ...
	       ctf_error_t *errp)
{
  ...
  if (ctf_dynhash_lookup_kv (arci->ctfi_named_indexes, name, NULL, &index_v) < 0)
    {
  ...
  if (!arci->ctfi_dict)
	...
	{
	  ctf_err (err_locus (NULL), EOVERFLOW,
		   _("overflow opening archive member %s; index %zi, but only %zi members"),
		   name, index, arci->ctfi_nmemb);
	  if (errp)
	    *errp = EOVERFLOW;
	  return NULL;
	}
      ...
      fp = ctf_dict_open_by_offset (arci, &arci->ctfi_symsect,
				    &arci->ctfi_strsect,
				    arci->ctfi_members[index], len, name,
...
ctf_dict_open_cached (struct ctf_archive_internal *arci, size_t index,
		      const char *name, ctf_error_t *errp)
{
  ctf_dict_t *fp;
  size_t len;
  ...
  len = ctf_arc_get_dict_len (arci, index);
  if ((fp = ctf_dict_open_by_offset (arci, &arci->ctfi_symsect,
				     &arci->ctfi_strsect,
				     arci->ctfi_members[index],
				     len, name,
		  ...
		  const char **name, int skip_parent, ctf_error_t *errp)
{
  ...
  f = ctf_dict_open_cached ((ctf_archive_t *) arci, index, name_, errp);
  return f;
